...
|---|---|
| **Platform architect** | [Core Concepts](../getting-started/concepts.md) → [Events](./events.md) → [mapexIam](./mapexos-core.md) |
...
| **Security reviewer** | [Security architecture](./security.md) → [mapexIam (Core)](./mapexos-core.md) |
...
Each service has its own page following the same template — *Purpose · Responsibilities · Internal architecture · Public contracts · Data model · Configuration · Observability · Operations · Security · Dependencies*.

- [**mapexIam (Core)**](./mapexos-core.md) — organizations, users, roles, JWT, RBAC cache
- [**http_gateway**](./http-gateway.md) — webhook ingestion, auth strategies, datasource registry
- [**assets**](./assets.md) — assets, templates, EVA fields, MQTT auth backend
- [**router**](./router.md) — match rules, fan-out, route groups
- [**events**](./events.md) — ClickHouse storage, NATS consumers, EVA queries
- [**triggers**](./triggers.md) — outbound executors, retries, DLQ
- **Workflow engine** — condition logic and workflows
- [**js-execution**](./js-execution.md) — V8 isolates, tiered cache, script lifecycle
- [**Security**](./security.md) — identity, isolation, threat model
