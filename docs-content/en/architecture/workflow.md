...
**Produces (NATS):** `…trigger.workflow.execute` → **Triggers** (plugin dispatch + `trigger_event`); `…workflow.js.code` → **JS Execution** (for `code` nodes); per-instance subworkflow executions; `workflow.state.>` lifecycle events (`WORKFLOW-STATE`) → the in-service **archiver**; and on terminal, `…events.workflow` (`EVENTS-WORKFLOW-LOGS`) → the **Events** service for permanent storage.
...
| Where `code` nodes run | [JS Execution](./js-execution.md) |
