...
type AssetTemplateCatalogRepository interface {
	...
	// GetItem returns the indexed catalog row for one template (identity/integrity
	// metadata such as marketplaceGuid and sha256), or ErrNotFound when unknown.
	GetItem(ctx context.Context, vendor, slug string) (*entities.CatalogItem, error)
